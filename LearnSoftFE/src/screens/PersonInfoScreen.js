...
export default  function PersonInfoScreen({ route, navigation }) {
  console.log("PERSONINFOSCREEN",route.params)
  const  user = route.params.props;
  const host = route.params.host;
  var role,department;
  console.log("SADASDASDASDASDASDA",user)
    if (user.userUnits.length != 0){
      role =user.userUnits[0].role
      department =user.userUnits[0].departmentName
    }else{
      role ="";
      department="";
    }
  ...
  return (
    <View style={{ flex: 1}}>
      ...
      <View style={styles.itemImage}>
        <Image source={require('./../../assets/lech-walesa.jpg')} style={styles.image}>
        </Image>
      </View>
