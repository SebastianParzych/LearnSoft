...
namespace LearnSoftBE.Data
{
    public class UserDataRepository : IUserDataRepository
    {
        ...
        public async Task<Chat> CreateNewChatAsync(Chat chat)
        {
            ...
            {
                part.ChatId = chat.ChatId;
                _context.UserChats.Add(part);
                _context.SaveChanges();
            }
         //   new_chat.Entity.Participants = await this.GerUsersListByChat(new_chat.Entity.ChatId);

            return await Task.FromResult(new_chat.Entity);
        }

        public async Task<IEnumerable<Message>> GetMessageListByChat(int chatId)
        {
            var messages= _context.Messages.AsNoTrackingWithIdentityResolution()
              .Where(p => p.ChatId == chatId).ToListAsync();
            return await messages;
        }

        public async Task<IEnumerable<UserChat>> GerUsersListByChat(int chatId)
        {
            var chat_participants=   _context.UserChats
               .AsNoTrackingWithIdentityResolution()
               .Include(p => p.User)
               .Where(p => p.ChatId == chatId).ToListAsync();

            return await chat_participants;
        }
